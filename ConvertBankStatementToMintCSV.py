...
def convertDBSCsv(source, outputFilePath):
    df=pd.read_csv("/Users/hannahchia/Downloads/DBSdata.csv",names=["Date", "Reference", "Debit Amount", "Credit Amount", "Transaction Ref1", "Transaction Ref2","Transaction Ref3"])
...
# Write main script
source = input("Input the source file (expected to be a CSV of DBS or SC format): \n")
outputFileName = input("Input the name of the file you want to create: \n")
bank = input("Input SC or DBS: \n")
while (bank != "SC" and bank != "DBS"):
    bank = input("Input was not valid. Input SC or DBS. \n")
